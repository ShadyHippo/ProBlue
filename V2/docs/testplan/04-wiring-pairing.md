...
these changes mandated by KEY_CONTEXT §2.
...
4. **GET_LTK experiment (now doc-backed, run as belt-and-braces)**: RE docs
   settle that step 2 returns the *stored* key (§2.6: "Acquire the XORed LTK
   ...
   If it ever differs → stop, re-open §2.6.
5. **The 3-step itself** — per KEY_CONTEXT §2.6 (XOR 0xAA, flash LE order,
...
- GET_LTK == stored-key confirmed on this unit (`25 31 28 85 …`); recorded as
  the doc-settled read-out behavior (§2.6).
...
Live-deploy fixes folded into the patch (see
[`docs/hardware/intel-9260-reconnect-wedge.md`](../hardware/intel-9260-reconnect-wedge.md)):
device is
...
- Patch: `src/patches/bluez-procon-cable-pairing-5.86.patch` (8 files:
...
- Build: `autoreconf -fi` + `./configure --enable-sixaxis …` + `make -j`
  per KEY_CONTEXT §1 recipe → `src/bluetoothd` (7.5 MB), zero warnings/errors.
